            ...
            return (
              <Card key={p.id} className="card-elevated">
                <CardContent className="p-4 flex items-center gap-3">
                  {p.photo_url ? (
                    <img src={p.photo_url} alt={p.name} className="w-14 h-14 rounded-xl object-cover shrink-0 shadow-sm" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-muted-foreground">Custo: <strong className="text-foreground">R$ {cost.toFixed(2)}</strong></span>
                      <span className="text-xs text-success">Lucro: <strong>R$ {profit.toFixed(2)}</strong></span>
                    </div>
                    <p className="text-sm font-extrabold text-success mt-0.5">Preço: R$ {price.toFixed(2)}</p>
                  </div>
